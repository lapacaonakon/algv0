...
export const TermPopover: React.FC<Props> = ({ anchor, onClose, onOpenSimulator: _onOpenSimulator, onCardEnter, onCardLeave }) => {
    ...
    >
      <div className="bg-slate-900 border border-indigo-500/40 rounded-xl shadow-2xl shadow-indigo-950/60 overflow-hidden">
        ...
        <div className="p-3 space-y-2.5 max-h-[60vh] overflow-y-auto overscroll-contain">
          ...
          {viz && (
            <div className="text-[11px] text-slate-400 bg-slate-800/50 border border-slate-700 rounded-lg px-2.5 py-1.5">
              Визуализация «{viz.title}» — в теории ниже, шаг в компиляторе подсвечивает её
            </div>
          )}
          <div className="text-[10px] text-slate-500 text-center pt-1">Нажми Esc или кликни вне, чтобы закрыть · тап на термин — закрепить</div>
        </div>
      </div>
      {pos && (
        <div
