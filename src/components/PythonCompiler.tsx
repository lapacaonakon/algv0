...
export function PythonCompiler({ chapterId }: Props) {
  ...
  const syncedStarter = meta && chapterId ? getMinimalStarter(chapterId) + "\n" : STARTER_CODE;
  ...
  const insertMinimal = () => {
    ...
    const mini = getMinimalStarter(chapterId);
    setCode(mini + "\n");
  };
  ...
  return (
    <main className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-10 py-6 lg:py-10">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-slate-400 mt-2 max-w-2xl leading-relaxed text-sm">
              Пиши код и жми <b className="text-white">Запустить</b> — Pyodide в браузере.
              {meta && (
                <span className="block mt-1 text-emerald-300 text-xs">
                  хардкод: <code className="bg-emerald-950 px-1 rounded">{syncedStarter.split("\n")[0]}</code> — минимум для понимания визуализации.
                </span>
              )}
            </p>
          </div>
        ...
        {liveVars && (
          <div className="mb-4 rounded-xl border border-emerald-600/40 bg-emerald-950/30 px-3 py-2 flex flex-wrap items-center gap-2 font-mono text-xs">
            <span className="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-300">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" /> WATCH
            </span>
            {Object.entries(liveVars).map(([k, v]) => (
              <span key={k} className="bg-slate-900 border border-slate-700 px-2 py-1 rounded text-slate-200">
                {k}={String(v)}
              </span>
            ))}
            <span className="ml-auto text-[10px] text-slate-400">шаг в визуализации слева = строка в терминале</span>
          </div>
        )}
        ...
        <div className="grid xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)] gap-5 items-start">
          <section className="rounded-2xl border border-slate-700 bg-slate-900/80 overflow-hidden shadow-xl shadow-black/10">
            <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-slate-800 bg-slate-900">
              ...
              <div className="flex items-center gap-2">
                {meta && (
                  <button type="button" onClick={insertMinimal} className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs text-indigo-300 hover:text-white hover:bg-indigo-900/50 transition-colors border border-indigo-800">
                    <Feather className="w-3.5 h-3.5" /> i,j
                  </button>
                )}
