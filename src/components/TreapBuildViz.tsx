...
const BuildMode: React.FC = () => {
  ...
  // набор точек из Python-кода пересобирает плоскость и дерево
  useEffect(() => {
    if (!live.pairsText || live.pairsText === raw) return;
    setRaw(live.pairsText);
    setPhase("connect");
    setOrder("y-desc");
    setSortIdx(3);
    setConnIdx(10000);      // все рёбра проведены — сразу видно итоговое дерево
    setPlaying(false);
...
const SPLIT_KEYS = [2, 3, 6, 7, 9, 12, 45];
...
const SplitMode: React.FC = () => {
  ...
  // mid из кода выбирает разрез, step листает кадры спуска
  useEffect(() => {
    if (live.mid === null || !SPLIT_KEYS.includes(live.mid) || live.mid === x0) return;
    setX0(live.mid);
    setStepIdx(0);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [live.mid]);
  useEffect(() => {
    ...
    setStepIdx(Math.max(0, Math.min(steps.length - 1, live.step)));
    // eslint-disable-next-line react-hooks/exhaustive-deps
...
const MergeMode: React.FC = () => {
  ...
  // step из кода листает кадры слияния
  useEffect(() => {
    if (live.step === null) return;
    setStepIdx(Math.max(0, Math.min(steps.length - 1, live.step)));
    // eslint-disable-next-line react-hooks/exhaustive-deps
...
const EraseMode: React.FC = () => {
  ...
  // key из кода выбирает удаляемую точку, step листает кадры
  useEffect(() => {
    if (live.key === null || !POINTS.some((p) => p.key === live.key) || live.key === key) return;
    setKey(live.key);
    setStepIdx(0);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [live.key]);
...
/**
 * Живые переменные Python-кода страницы treap:
 *  points / pairs — набор пар (x; y): по нему ПЕРЕСТРАИВАЕТСЯ всё дерево;
 *  mid            — разрез для вкладки Split;
 *  key            — удаляемая точка для вкладки Erase;
 *  step / i       — номер шага (листает кадры Split / Merge / Erase);
 *  height, n      — высота дерева и число узлов (показываются в шапке).
 */
const useTreapLive = () => {
  ...
  const pairsText = useMemo(() => {
    if (!vars) return null;
    const rawPts = vizArray(vars.points) ?? vizArray(vars.pairs) ?? vizArray(vars.items);
    if (!rawPts || rawPts.length === 0) return null;
    const parts: string[] = [];
    rawPts.forEach((it) => {
      if (Array.isArray(it) && it.length >= 2) {
        const x = vizNumber(it[0]);
        const y = vizNumber(it[1]);
        if (x !== null && y !== null) parts.push(`${x} ${y}`);
      } else if (typeof it === "string") {
        const m = it.match(/-?\d+(?:\.\d+)?/g);
        if (m && m.length >= 2) parts.push(`${m[0]} ${m[1]}`);
      }
    });
    return parts.length ? parts.join(", ") : null;
  }, [vars]);

  const mid = vars ? vizNumber(vars.mid) : null;
  const key = vars ? vizNumber(vars.key) : null;
  const step = vars ? vizNumber(vars.step) ?? vizNumber(vars.i) : null;
  const height = vars ? vizNumber(vars.height) ?? vizNumber(vars.h) : null;
  const n = vars ? vizNumber(vars.n) : null;
  return { vars, pairsText, mid, key, step, height, n };
};
...
export const TreapBuildViz = () => {
  ...
  // вкладка = подрежим страницы: панель Python подставляет свой референсный код
  useEffect(() => {
  ...
  return (
    <div className="bg-slate-800/90 p-6 rounded-2xl border border-slate-700 shadow-xl max-w-6xl mx-auto my-4">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        {live.vars ? (
          <>
            {live.pairsText && <span className="text-[11px] font-mono bg-slate-950 border border-emerald-600/40 text-emerald-300 rounded-md px-2 py-1">points = {live.pairsText}</span>}
            {live.n !== null && <span className="text-[11px] font-mono bg-slate-950 border border-slate-700 text-slate-300 rounded-md px-2 py-1">n = {live.n}</span>}
            {live.height !== null && <span className="text-[11px] font-mono bg-slate-950 border border-slate-700 text-slate-300 rounded-md px-2 py-1">height = {live.height}</span>}
            {live.mid !== null && <span className="text-[11px] font-mono bg-slate-950 border border-sky-600/40 text-sky-300 rounded-md px-2 py-1">mid = {live.mid}</span>}
            {live.key !== null && <span className="text-[11px] font-mono bg-slate-950 border border-rose-600/40 text-rose-300 rounded-md px-2 py-1">key = {live.key}</span>}
          ...
          <span className="text-[11px] text-slate-500">
            открой панель Python: переменные <code className="font-mono text-emerald-300">points</code>, <code className="font-mono text-emerald-300">mid</code>, <code className="font-mono text-emerald-300">key</code>, <code className="font-mono text-emerald-300">step</code> управляют этой демонстрацией
          </span>
        )}
      </div>
