        ...
        <div className="mt-2 text-xs text-slate-300 bg-slate-950/70 rounded-lg px-3 py-2 border border-slate-800 min-h-[36px]">
          {phase === "sort" ? (
            algo === "none" ? (
              ...
              <>Отсортировано по y. Смета сортировки: quicksort ≈ <b>{n > 1 ? Math.round(n * Math.log2(n)) : 0}</b> сравнений, counting — <b>0</b> (n+k операций). Дерево в обоих случаях одно и то же. Теперь соединяй →</>
            )
          ) : phase === "game" ? (
            gameWon ? (
