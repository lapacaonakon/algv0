 ...
 *
 * Поднят с 500 до 1500: эталонный код Флойда—Уоршелла на демо-графе 7×7
 * делает 869 шагов, «Классы сложности» — 585, Ахо—Корасик — 480. При старом
 * лимите трасса обрывалась на середине алгоритма, и визуализация не получала
 * итоговую матрицу. Замер: 869 шагов Флойда выполняются за ~120 мс.
 */
export const DEBUG_STEP_CAP = 1500;
...
def __capture(frame):
    # Фрейм модуля: f_locals — это тот же словарь, что и f_globals. Читать его
    # на каждом событий строки НЕЛЬЗЯ: в CPython 3.12 встроенные (PEP 709)
    # списковые включения шлют событие на своей строке каждую итерацию, а
    # обращение к f_locals между итерациями обнуляет переменные перечисления —
    # код вида E = [(a, b) for a in V for b in V if a[0] < b[0]] падал с
    # TypeError: 'NoneType' object is not subscriptable и сыпал
    # RuntimeWarning: assigning None to unbound local.
    if frame.f_code.co_name == "<module>":
        scope_values = __visible(frame.f_globals.items())
        return (
            {k: __safe(v) for k, v in scope_values.items()},
            {k: __snapshot(v) for k, v in scope_values.items()},
        )
...
def __tracer(frame, event, arg):
    ...
    # f_lineno == 0 — это RESUME модуля, а не строка исходника: в CPython 3.12
    # событие приходит даже для файла, где одни комментарии. Без фильтра такой
    # код давал «шаг 1/1», и панель прятала редактор за пустым листингом.
    if event == "line" and frame.f_code.co_filename == "<user-code>" and frame.f_lineno > 0 \
