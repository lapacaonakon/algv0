    ...
    <div class="space-y-8">
        ...
        <div class="bg-slate-700/50 p-6 rounded-xl border-l-4 border-blue-500 scroll-mt-10">
            ...
            <div class="bg-slate-900 p-4 rounded-lg mb-6 border border-blue-500/30">
                ...
                <p class="text-slate-300 text-sm mt-3"><b class="text-amber-300">Сложность — явно:</b> одна операция — до <code class="bg-slate-950 px-2 py-0.5 rounded text-xs font-mono text-rose-300 border border-slate-800">O(n)</code>; m операций подряд — <code class="bg-slate-950 px-2 py-0.5 rounded text-xs font-mono text-emerald-300 border border-slate-800">O(m·log n)</code> всего, т.е. амортизированное <code class="bg-slate-950 px-2 py-0.5 rounded text-xs font-mono text-emerald-300 border border-slate-800">O(log n)</code> на операцию; память — <code class="bg-slate-950 px-2 py-0.5 rounded text-xs font-mono text-emerald-300 border border-slate-800">O(n)</code> (ключ + 2 ссылки).</p>
            </div>
...
обход слева направо ПОСЛЕ:  A 20 B 40 C     ← тот же самый!</pre>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                ...
                <div class="bg-slate-900 rounded-lg p-4 border border-amber-700/50">
                    ...
                    <p class="text-slate-300 text-xs">Поддерево B висело справа от 20 и слева от 40 — то есть его ключи между 20 и 40. Это одно и то же место, поэтому B просто перевешивается с одного на другое. Только оно и меняет хозяина. Как вправление вывиха: одна кость (x) ходит вокруг другой (p), а «суставной груз» B перетекает с низа на верх — потому между A и B и «ничего не видно» на мгновенных схемах: движение происходит в одной точке, всё остальное стоит.</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-slate-800/60 p-6 rounded-xl border border-slate-700">
            ...
            <p class="text-slate-300 text-sm mt-4">И так по кругу: определили случай → сделали 1–2 поворота → x поднялся ближе к корню → снова смотрим на тройку. Zig может случиться только один раз — самым последним, если до корня остался ровно один шаг.

            <div class="text-center my-5 bg-slate-950/70 rounded-xl border border-indigo-500/40 px-4 py-4">
                <p class="text-xl sm:text-2xl font-bold text-white">ЛИНИЯ → крути <span class="text-amber-300">СВЕРХУ</span> &nbsp;·&nbsp; ЗМЕЙКА → крути <span class="text-amber-300">СНИЗУ</span></p>
                <p class="text-slate-400 text-xs mt-2">Zig-Zig — линия (лево-лево или право-право). Zig-Zag — змейка. Порядок — всё: он и отличает splay от «просто поворотов».</p>
            </div></p>
            <p class="text-slate-400 text-xs mt-3">👇 Ниже на этой странице интерактив на большом дереве (14 узлов): кликни узел — и его подъём в корень разложится на атомарные кадры: <b>прицел на ребро</b> → <b>отстёгиваем среднее поддерево</b> (пунктиром показано, какой груз переедет) → <b>поворот</b> (зелёным горят переехавшие рёбра). Рядом с логом — врезка «Механика одного поворота» (схема p↔x с α/β/γ), а под демо — песочница: подними узел сам по одному повороту за клик, и проверка сверит твой порядок со splay.</p>
        </div>
