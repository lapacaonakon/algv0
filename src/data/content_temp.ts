...
void dfs(int v, int p = -1) {
    ...
    if (p == -1 && children > 1)
        ...
        <div class="bg-indigo-950/60 p-6 rounded-xl border-l-4 border-indigo-500 mt-6">
            <h4 class="text-lg font-bold text-indigo-400 mb-2">🧠 Различие концепций: Неориентированные vs Ориентированные графы</h4>
            <p class="text-slate-300 text-sm mb-4">
                Важно чётко различать тип графа при решении задач связности:
            </p>
            <ul class="list-disc list-inside text-sm text-slate-300 space-y-2">
                <li><strong class="text-rose-400">Точки сочленения и мосты (Билеты 11–12)</strong> определены ИСКЛЮЧИТЕЛЬНО для <strong>неориентированных графов</strong>. Они описывают связность и блоки (двусвязные компоненты).</li>
                <li><strong class="text-emerald-400">SCC и конденсация (Билет 10)</strong> определены ИСКЛЮЧИТЕЛЬНО для <strong>ориентированных графов</strong>. Сжатие SCC даёт ориентированный ациклический граф (DAG).</li>
                <li><strong>Внимание:</strong> Нельзя путать эти понятия! Конденсация SCC и точки сочленения — это абсолютно разные математические структуры для разных типов графов.</li>
            </ul>
        </div>
    </div>
</section>`,
