        ...
        <div class="bg-slate-700/50 p-6 rounded-xl border-l-4 border-emerald-500 scroll-mt-10">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
                ...
                <div class="bg-slate-900 p-6 rounded-lg border-2 border-dashed border-sky-500/50 relative pt-8">
                    <div class="absolute -top-3 left-4 bg-slate-700 text-sky-300 text-xs px-3 py-1 rounded-full font-bold uppercase border border-sky-500 shadow-md">
                        🔋 Аналогия 2: Аккумулятор
                    </div>
                    <p class="text-slate-300 text-sm mb-4">Префиксная матрица «тонкая» — ровно одна копия на клетку — потому что сумма <b>аккумулирует</b>: каждая клетка P[i][j] уже несёт в себе площадь всего прямоугольника от угла (0,0). Лишнее просто <b>вычитаем</b> по включениям-исключениям. Фокус держится на обратной операции: у минимума «вычесть» нет, поэтому там (билет 2) приходится печь слёный тортик из ~100 копий матрицы.</p>
                </div>
            </div>
