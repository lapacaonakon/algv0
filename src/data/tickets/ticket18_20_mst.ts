...
<section id="mst" class="mb-12 scroll-mt-10">
    ...
    <div class="space-y-8">
        <div class="bg-slate-700/50 p-6 rounded-xl border-l-4 border-emerald-500 scroll-mt-10">
            ...
            <p class="text-slate-300 text-sm mb-4">Все три алгоритма — <b>жадные</b>, и все три опираются на одну теорему. <b>Свойство разреза (cut property):</b> для любого разреза <span class="font-mono">(S, V \ S)</span> самое лёгкое ребро, пересекающее разрез, входит в некоторое MST. Алгоритмы отличаются тем, <b>какой разрез</b> они выбирают: Краскал берёт минимальное ребро между двумя компонентами текущего леса (разрез — компонента против всех), Прим растёт от текущей компоненты дерева, а Борувка строит разрезы для всех компонент одновременно.</p>
            ...
            <p class="text-slate-300 text-sm mb-4">Второй инструмент — <b>свойство цикла (cycle property):</b> <b>строго самое тяжёлое</b> ребро в любом цикле никогда не входит ни в одно MST. (Если в цикле несколько рёбер с одинаковым максимальным весом, по крайней мере одно из них не входит в данный MST). Именно его использует Краскал, отбрасывая рёбра, замыкающие цикл.</p>
            ...
            <div class="bg-slate-900/40 rounded-lg border border-slate-700/50 p-4 mt-6">
                ...
                <p class="text-slate-400 text-sm mb-1">· <b>«MST единственно»</b> — попарная различность весов рёбер является <b>достаточным</b>, но НЕ необходимым условием единственности MST (граф с одинаковыми весами также может иметь единственное MST).</p>
...
print(total)</pre>
            ...
            <details class="bg-slate-900/40 rounded-lg border border-slate-700/50 group cursor-pointer mt-4">
                ...
                <div class="p-5 text-sm text-slate-300 space-y-2">
                    ...
                    <p>· Что если граф несвязный? — Прим за один запуск от $s$ построит остов только для одной компоненты; для построения минимального остовного <b>леса</b> (MSF) Прим нужно обернуть во внешний цикл по непосещённым вершинам. Краскал и Борувка строят лес автоматически.</p>
