...
/**
 * Минимальный хардкод вместо слов — строки которые за минимум информации
 * дают понять визуализацию. Код отсутствует нарочно: только i,j и 4→1.
 *
 * 1D: i=0, j=0 — при пошаговом выполнении подсветится первый элемент
 * 2D: a,b,c,d = new — 4 ячейки склеиваются в одну (2×2 блок)
 */
export function getMinimalStarter(id: string): string {
  const meta = getFallbackMeta(id);
  if (id === "sparse-table") {
    return `i, j = 0, 0  # 1D: первый блок\na, b, c, d = 4, 1, 3, 2  # 2D: 2×2 → new\nans = min(a, b, c, d)  # new`;
  }
  ...
  return `n, m = ${meta.vars.n?.example ?? 0}, ${meta.vars.m?.example ?? 0}  # ${meta.title}\ni, j = 0, 0`;
}

/** Полный код алгоритма — показывается по кнопке "показать код" под визуализацией */
export function getFullCode(id: string): string | null {
  if (id === "sparse-table") {
    return `# Разреженная таблица — построение
# Визуализация — отдельный объект: SparseTableVars + SparseTableCode
for j in range(1, LOG):
    for i in range(n - (1<<j) + 1):
        ST[i][j] = min(ST[i][j-1], ST[i + (1<<(j-1))][j-1])  # i,j
...
for kx in range(4):
  for ky in range(4):
    ST[r][c][kx][ky] = min(
        ST[r][c][kx-1][ky], ST[r+(1<<(kx-1))][c][kx-1][ky])  # по вертикали
    # иначе по горизонтали: min(ST[r][c][kx][ky-1], ST[r][c+(1<<(ky-1))][kx][ky-1])
`;
  }
  if (id === "prefix-sums-2d") {
  ...
  for i in range(n):
    for j in range(n):
      if dist[i][k]+dist[k][j] < dist[i][j]:
        dist[i][j] = dist[i][k]+dist[k][j]  # k,i,j`;
  }
  return null;
}
