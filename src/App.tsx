...
import { LiteGuide } from "./components/LiteGuide";
import { PythonCompiler } from "./components/PythonCompiler";

/**
 * «Лёгкий» режим для печати и внешних парсеров: `?lite=1` (всё пособие)
 * или `?lite=1&topic=mst` (одна тема). Без навигации, демонстраций и компилятора.
 */
function readLiteMode(): { lite: boolean; topic?: string } {
  if (typeof window === "undefined") return { lite: false };
  const q = new URLSearchParams(window.location.search);
  const lite = q.has("lite") || q.get("view") === "lite" || q.get("print") === "1" || q.get("text") === "1";
  return { lite, topic: q.get("topic") ?? undefined };
}

export default function App() {
  const liteMode = useMemo(readLiteMode, []);
  const [activeChapterId, setActiveChapterId] = useState<string>(() => {
    const q = typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("topic") : null;
    return q && chapters.some((c) => c.id === q) ? q : chapters[0].id;
  });
  ...
  // Текстовая версия: её печатают в PDF и читают внешние парсеры.
  if (liteMode.lite) return <LiteGuide topicId={liteMode.topic} />;

  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-indigo-500/30">
      <Navbar compilerOpen={compilerOpen} onToggleCompiler={() => setCompilerOpen((o) => !o)} />
      ...
      <div ref={contentRef} className={compilerOpen ? "max-lg:pb-[58dvh]" : ""}>
        ...
        <footer className="p-8 text-center text-slate-600 text-xs border-t border-slate-900 mt-12 bg-slate-950 print:hidden">
